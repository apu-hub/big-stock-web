...
$imgid = Utils::cleaner($_POST['id'] ?? 0);
$hashID = Utils::cleaner($_POST['hash_id'] ?? "");
$data = array(
	"category_id" => Utils::cleaner($_POST['category_id'] ?? ""),
	"tag_id" => Utils::cleaner($_POST['tag_id'] ?? ""),
	"image_name" => Utils::cleaner($_POST['image_name'] ?? ""),
	"image_description" => Utils::cleaner($_POST['image_description'] ?? ""),
	"image_price" => Utils::cleaner($_POST['image_price'] ?? ""),
	"image" => Utils::cleaner($_POST['image_old'] ?? ""),
	"file" => Utils::cleaner($_POST['raw_old'] ?? ""),
);
...
if ($_FILES['raw']['name'] != '') {
	...
	$data['file'] = $raw_file_name;
}

$obj->update($table, $data, $imgid);
$session->with("info", "Image Updated");
// Utils::phpRedirect('showimage.php');
Utils::phpRedirect('editimage.php?id=' . $imgid);
